...
## WS6 — Tiers de relació IG (DMs/DYI) (2026-06-15)

Anàlisi read-only de l'export **Download Your Information** d'Instagram
(`@topquaranta`, 2026-06-15). L'API no serveix per a aquest senyal
(comentaris/DMs/mencions de tercers tornen buits sota Standard Access;
verificat encunyant un token amb els 5 scopes — segueix buit; cal
Advanced Access/App Review). El DYI sí dóna els DMs sencers de les dues
bandes.

Dades crues i scripts: `instagram-topquaranta-2026-06-15-UjEDFnAF/`
(`_EXTRET/` = volcat cru; `_EXTRET/_TIERS/` = perfils + proposta). Tot
fora del codi del repo.

Fets (mètode i evidència a `_TIERS/methodology.md`):

- **1.790 comptes externs** amb relació; **661** amb DM; **153** creuats
  com a artistes (match exacte `norm(username)` ↔ `Artista.instagram_url`,
  sense fuzzy). Connexions: 747 followers / 678 following / **316 mutus**.
- Tres famílies de senyal separades i **direcció de shares no
  col·lapsada** (`shares_in` ells→TQ vs `shares_out` TQ→ells; 173 entrants,
  73 de contingut topquaranta).
- Distribució de profunditat de conversa (DM): p50=1, p90=6, p99=26 —
  relació típica fina, cua que importa.
- Proposta de **6 tiers candidats** (waterfall per profunditat +
  reciprocitat + direcció d'inversió): T1 nucli=30, T2 activa=77,
  T3 caliu=23, T4 inversió-unidireccional=137, T5 contacte-puntual=416,
  T6 latent=1.107. `_TIERS/tiers_proposal.md`.
- **Descobriment** (117 comptes alta-implicació no-fitxats):
  `_TIERS/discovery.csv`. Inclou bandes reals sense match (Duplicats,
  Marjal, Apología, Güil…), connectors d'indústria
  (`ooriginalmanagement`, mitjans `rockimes`/`primerafila.cat`/…) i fans
  forts. Avís: alguns "no-fitxats" són artistes amb handle de DM ≠
  `instagram_url` (masvidal, abril), **no entitats noves**.
- **Media nodes:** Tardor (Alex Blat) **actiu** (últim 2026-06-13); Llet i
  Vi (Obrim Portes) **fred** (últim 2025-10-27); Tresdeu (Oscar) **sense
  relació** (només follow, 0 missatges). Detall a
  `_TIERS/tiers_proposal.md §5`.

Límits: comentaris rebuts als posts de TQ absents (DYI i API);
interaccions d'stories només sortints; `liked_posts` no atribuïble per
username. **Sense decisions:** la definició final de tiers i les accions
són d'en Miquel.
