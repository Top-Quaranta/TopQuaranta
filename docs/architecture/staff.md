...
| GET | `/staff/social/matriu/` | **(2026-06)** Distribution matrix (third gate, `MatriuPublicacio`): `{canals, tipus, dies, cells:[{canal, tipus, actiu, dia_setmana, seeded}]}`. Non-seeded combos report `seeded=False` (fail-open, not a real slot). See `social.md`. |
| POST | `/staff/social/matriu/toggle/` | **(2026-06)** Set one matrix cell. Body: `canal`, `tipus`, and `actiu?` (defaults to flipping) and/or `dia_setmana?` (int 0-6 or null = no weekday restriction). A day edit never flips `actiu`. Only seeded cells; `actiu=False` on (newsletter × top_ppcc) stops the Sunday send. |
