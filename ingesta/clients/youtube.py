...
def video_stats(video_ids: list[str]) -> dict[str, dict]:
    """`{video_id: {"views", "likes", "published_at"}}` for up to 50 ids.

    One quota unit per call regardless of how many ids **or how many
    parts**, which is what makes a daily poll of the whole catalogue
    affordable — `snippet` rides along free. `likeCount` is absent when
    the uploader hides it; views are what we actually use.

    `published_at` (a `date`) is the age evidence the ranking needs: an
    Art Track is generated by the distributor at release, so it dates
    the recording. 500-song sample on 2026-08-22: 93,4 % within ±30 days
    of the release date we hold, and the 2 % published over a year
    earlier were remasters and re-issues wearing a new date.
    """
    if not video_ids:
        return {}
    data = _get("videos", part="statistics,snippet", id=",".join(video_ids[:50]))
    out: dict[str, dict] = {}
    for item in data.get("items", []):
        stats = item.get("statistics", {})
        views = stats.get("viewCount")
        likes = stats.get("likeCount")
        publicat = (item.get("snippet") or {}).get("publishedAt") or ""
        out[item["id"]] = {
            "views": int(views) if views is not None else None,
            "likes": int(likes) if likes is not None else None,
            "published_at": _data_iso(publicat),
        }
    return out


def _data_iso(valor: str) -> datetime.date | None:
    """`2026-08-20T18:00:00Z` → `date(2026, 8, 20)`. None si no s'entén."""
    try:
