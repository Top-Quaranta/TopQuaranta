...
# during the retrain (until 2026-09 a thread inside gunicorn, now the
# nightly `recalcular_ml` cron), there is nothing for us to act on, and
# between the 9th and
...
# Newsletter cloud-routine bearer token (2026-06-07). The routine reads
# the weekly brief and leaves a draft via two token-authed endpoints
# (NOT staff session). The token is generated by an operator and placed
# in the server environment; production.py reads it via `config(...)`.
# Empty default (here + test/local) means token auth is effectively
# disabled — the permission denies every request when the token is blank,
# so a misconfigured deploy never accepts an empty bearer.
NEWSLETTER_ROUTINE_TOKEN = ""

# VilaWeb RSS feed for the brief's "actualitat" section (best-effort,
# read at request time with a short timeout; a failure yields an empty
# section, never a 500).
VILAWEB_RSS_URL = "https://www.vilaweb.cat/feed/"
