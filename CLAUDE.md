...
- **`docs/archive/decisions/`** — ADRs (Architecture Decision Records).
- **`docs/archive/post-mortems/`** — incident write-ups.
...
| **Public read cache (2026-04-25)** | Hot read endpoints `/api/v1/{ranking,artistes,mapa/artistes-top}/` cached **60 s for anonymous hits** in `pagecache` (LocMem per worker). Authenticated requests bypass. Each endpoint also exposes ETag + Last-Modified via Django's `condition` decorator (rooted at `TopProvisional.data_calcul`, `Artista.created_at`, `SenyalDiari.data` respectively) — re-fetching clients get a 304 in ~5 ms. Helper at `web/api/utils.py::cache_for_anon`. |
...
| **Gunicorn `--reload` (REMOVED 2026-05-20)** | Vegeu `docs/archive/decisions/0001-gunicorn-no-reload.md` (Accepted 2026-05-20). |
| **Mail infrastructure (2026-08-18)** | Les bústies de `topquaranta.cat` i `cercol.team` viuen a **Purelymail** (MX `mailserver.purelymail.com`); IMAP `imap.purelymail.com:993`, SMTP `smtp.purelymail.com:465`, tot amb la contrasenya de la bústia. L'enviament automàtic continua per **Brevo** (TopQuaranta) i **Resend** (Cercol), que és el que diuen els SPF. **Stalwart s'ha retirat**: la caixa de Hetzner ja no allotja correu, `mail.topquaranta.cat` respon 410 i només serveix l'autoconfig. La raó de fons per la qual existia el smarthost —Hetzner bloqueja el port 25 d'eixida— continua sent certa i és per què no es torna a allotjar correu ací. Arquitectura a `docs/EMAIL.md`; el post-mortem del certificat caducat (`docs/archive/post-mortems/2026-07-26-stalwart-cert-expirat.md`) es conserva perquè la seua lliçó —mesurar el certificat pel cable, mai llegint el fitxer— val per a qualsevol servei. |
...
| **Narrative engine for weekly social posts (Fase 4 PR 1, 2026-05-18; wired since)** | `social/narrative/` package — **wired into `captions.py::compose_for_channel`** for tipus `top_ppcc` / `top_territorial`. Expanded to **13 detectors** (a1-a13 + fallback) over `TopSetmanal`. Anti-repeat registry backed by `social.NarrativePhraseUsage`. Per-channel composers apply mention style and length budgets. L'IG `@handle` sí que funciona al camí narratiu: la regressió del 2026-05-20 (`docs/archive/post-mortems/2026-05-20-narrative-engine-collapsed.md`) es va arreglar l'endemà a #59 amb l'ADR-0007, que el reintrodueix al composer d'instagram_feed (`_ig_label`). Verificat per mutació el 2026-08-16. Full architecture at `docs/architecture/social.md`. |
...
`docs/archive/decisions/0003-pytest-ds-pin.md`.
...
- Update the doc when the change alters what the doc DESCRIBES
  ...
  criteria + examples at `docs/policies/conventions.md`.
