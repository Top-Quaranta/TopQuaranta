...
| **Gunicorn `--reload` (REMOVED 2026-05-20)** | Was added 2026-04-27 to pick up renderer edits without reload. **Removed** after three production 500s in one week (`imatge_url`, `reconsiderada`, `SolicitudRevisio`) caused by workers picking up uncommitted edits before migrations had been applied. Deploy now goes exclusively through `bin/tq-deploy` (git reset → migrate → SIGHUP). Vegeu `docs/decisions/0001-gunicorn-no-reload.md` (Accepted 2026-05-20) i `docs/post-mortems/2026-05-19-gunicorn-reload-incidents.md`. |
