...
| Instagram suggestion buffer (2026-07-29, migration `music 0094`) | `instagram_url_suggerit` (URL) + `instagram_suggerit_nota` (CharField 200, the evidence shown to staff). **Not** in `SOCIAL_LINK_FIELDS` and read by nothing but the staff queue: a candidate handle found out-of-band lands here, never in `instagram_url`, because that field drives public IG `user_tags` + collaborator invitations and a wrong handle tags a stranger on a live post. Promotion is a staff click; the PATCH that fills `instagram_url` clears both. Migration `0094` seeds the first 29 candidates (idempotent, skips rows that already have an `instagram_url`). |
