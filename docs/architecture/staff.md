...
| — | *(filtre `youtube`, 2026-08)* | `pendent` / `revisat`, sobre `Artista.youtube_canal_revisat`. **Tres estats, no dos:** un artista revisat i confirmat com a «no té canal propi» està FET, no pendent, i no ha de tornar a la cua per sempre (Malalts no en té). Alimenta `/staff/artistes/sense-youtube`. |
