...
| GET | `/staff/social/matriu/` | **(2026-06)** Distribution matrix (third gate, `MatriuPublicacio`): `{canals, tipus, cells:[{canal, tipus, actiu, seeded}]}`. Non-seeded combos report `seeded=False` (fail-open, not a real slot). See `social.md`. |
| POST | `/staff/social/matriu/toggle/` | **(2026-06)** Set one matrix cell. Body: `canal`, `tipus`, `actiu?` (defaults to flipping). Only seeded cells; `actiu=False` on (newsletter × top_ppcc) stops the Sunday send. |
